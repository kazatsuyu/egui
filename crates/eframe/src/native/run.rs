...
fn with_event_loop<R>(
    ...
    f: impl FnOnce(EventLoop<UserEvent>, epi::NativeOptions) -> R,
) -> R {
    ...
    EVENT_LOOP.with(|event_loop| {
        ...
        f(
            std::mem::replace(event_loop, create_event_loop(&mut native_options)),
            native_options,
        )
    })
}
    ...
    event_loop
        .run(move |event, event_loop| {
            ...
            if let Some(next_repaint_time) = next_repaint_time {
                ...
                event_loop
                    .set_control_flow(winit::event_loop::ControlFlow::WaitUntil(next_repaint_time));
            };
        })
        .unwrap();
    let exit = std::process::exit;
    exit(0);
}
